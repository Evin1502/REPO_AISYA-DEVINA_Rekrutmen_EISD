...
@section('title', 'Tambah Reward - Admin TemJi')
    ...
    <div class="mx-auto max-w-xl">
        ...
        <div class="card">
            <div class="card-header">
                <h1 class="text-lg font-bold text-on-surface">Tambah Reward</h1>
            </div>
            <div class="card-body">
                <form method="POST" action="{{ route('admin.rewards.store') }}" enctype="multipart/form-data" class="space-y-5">
                    ...
                    <div>
                        ...
                        <select id="category" name="category" required
                            ...
                            <option value="saldo" @selected(old('category', 'saldo') === 'saldo')>Saldo / Cash Balance</option>
                            <option value="barang" @selected(old('category') === 'barang')>Barang Fisik</option>
                        </select>
                        @error('category')
                            <div class="form-error">{{ $message }}</div>
                        @enderror
                    ...
                    <div id="nominalField">
                        ...
                        <input type="number" id="nominal" name="nominal" min="0" step="1" value="{{ old('nominal') }}"
                    ...
                    <div>
                        ...
                        <input type="number" id="points_required" name="points_required" min="1" value="{{ old('points_required') }}" required
                    ...
                    <div>
                        <label for="filepond" class="form-label">Gambar (opsional)</label>
                        <input type="file" id="filepond" name="image" accept="image/*"
                               class="form-control @error('image') input-error @enderror">
                        @error('image')
                            <div class="form-error">{{ $message }}</div>
                        @enderror
                        <div class="form-text">JPG, PNG, GIF, atau WEBP maksimal 2 MB.</div>
                    </div>

                    <div>
                        <label for="description" class="form-label">Deskripsi (opsional)</label>
                        <textarea id="description" name="description" rows="3" maxlength="1000"
                                  class="form-control @error('description') input-error @enderror">{{ old('description') }}</textarea>
                        @error('description')
                            <div class="form-error">{{ $message }}</div>
                        @enderror
                    </div>
