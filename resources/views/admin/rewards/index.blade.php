...
@section('content')
    <div class="mb-4 flex items-center justify-between">
        <div>
            <h1 class="text-xl font-bold text-on-surface">Reward</h1>
            <p class="text-sm text-on-surface-variant">Kelola katalog reward untuk penukaran poin.</p>
        </div>
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
            @foreach ($rewards as $reward)
                <div class="card flex flex-col overflow-hidden">
                    ...
                    @else
                        <div class="flex h-40 items-center justify-center bg-gradient-to-br from-surface-container-low to-surface-container-high text-5xl">🎁</div>
                    @endif
                    <div class="flex flex-1 flex-col p-5">
                        <div class="flex items-center justify-between gap-2">
                            <h3 class="text-lg font-bold text-on-surface">{{ $reward->name }}</h3>
                            <span class="shrink-0 rounded-full bg-surface-container px-2.5 py-0.5 text-xs font-semibold text-on-surface-variant">
                                {{ $reward->categoryLabel() }}
                            </span>
                        </div>
                        <p class="mt-1 flex-1 text-sm text-on-surface-variant">{{ $reward->description ?? 'Tanpa deskripsi.' }}</p>
                        <div class="mt-3 flex items-center justify-between text-sm">
                            ...
                            <span class="text-xs text-outline">{{ $reward->point_exchanges_count }} penukaran</span>
                        </div>
