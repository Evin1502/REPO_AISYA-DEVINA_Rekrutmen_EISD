...
@section('content')
        ...
        <div class="mx-auto max-w-7xl px-5 sm:px-8 lg:px-12">
            <div class="grid grid-cols-1 items-center gap-12 lg:grid-cols-12 lg:gap-10">
                <div class="lg:col-span-6">
                    ...
                    <h1 class="mb-6 font-display text-4xl font-extrabold leading-[1.12] tracking-tight text-ink sm:text-5xl lg:text-[58px]">
                    ...
                    <p class="mb-10 max-w-xl text-lg font-normal leading-relaxed text-on-surface-variant sm:text-xl">
                        TemJi membantu kamu memahami dampak sampah, memilah dengan benar, dan mengubahnya menjadi <strong class="font-bold text-ink">poin dan reward nyata</strong>.
                    </p>
                    <div class="mb-10 flex flex-wrap items-center gap-4 sm:gap-6">
                        ...
                        <a class="group inline-flex items-center gap-3 rounded-full px-5 py-3 text-base font-semibold text-on-surface transition-all hover:bg-white/80 hover:text-brand-700" href="#dampak">
                            <span class="flex h-10 w-10 items-center justify-center rounded-full border border-outline-variant/40 bg-white text-brand-600 shadow-sm transition-all group-hover:scale-105 group-hover:border-brand-500">
                    ...
                    <div class="flex flex-wrap items-center gap-4 text-xs font-semibold text-on-surface-variant">
                        ...
                        <span>Bergabung bersama <strong class="text-ink">{{ number_format($stats['residents']) }} warga</strong> yang sudah terdaftar</span>
                    </div>
                </div>

                <div class="relative flex justify-center lg:col-span-6 lg:justify-end">
                    <div class="relative w-full max-w-[510px]">
                        <div class="relative z-10 rounded-3xl border border-outline-variant/40 bg-white p-6 shadow-dashboard sm:p-8">
                            <div class="flex items-start justify-between border-b border-outline-variant/40 pb-6">
                                <div>
                                    <div class="mb-1 flex items-center gap-2">
                                        <span class="text-xs font-bold uppercase tracking-wider text-outline">Total poin dibagikan</span>
                                    ...
                                    <div class="flex items-baseline gap-2">
                                        <span class="font-display text-3xl font-black tracking-tight text-ink sm:text-4xl">{{ number_format($stats['total_points']) }}</span>
                                        <span class="text-base font-bold text-outline">Poin</span>
                                    </div>
                            ...
                            <div class="my-1 py-4">
                                ...
                                <div class="space-y-2.5">
                                    @forelse ($wasteCategories->take(3) as $i => $category)
                                        <div class="flex items-center justify-between rounded-xl border border-outline-variant/40 bg-paper p-2.5">
                                            <div class="flex items-center gap-2.5">
                                                ...
                                                <div>
                                                    <p class="text-xs font-bold leading-tight text-ink">{{ $category->name }}</p>
                                                    <p class="text-[11px] text-on-surface-variant">{{ \Illuminate\Support\Str::limit($category->description, 36) }}</p>
                                                </div>
                                            </div>
                                            <span class="font-mono text-xs font-extrabold text-brand-700">+{{ $category->points_per_kg }}</span>
                                        </div>
                                    @empty
                                        <p class="text-sm text-on-surface-variant">Kategori sampah belum ditambahkan.</p>
                                    @endforelse
                                </div>
                            </div>
                            <div class="grid grid-cols-2 gap-3 pt-3">
                                <div class="rounded-2xl border border-outline-variant/40 bg-surface-container-low p-3.5">
                                    <div class="mb-1 flex items-center gap-1.5 text-xs font-medium text-on-surface-variant">
                                        <span class="material-symbols-outlined text-[16px] text-emerald-600">recycling</span>
                                        Total didaur ulang
                                    </div>
                                    <p class="text-lg font-black text-ink">{{ number_format($stats['total_weight'], 1) }} <span class="text-xs font-bold text-on-surface-variant">kg</span></p>
                                </div>
                                <div class="rounded-2xl border border-outline-variant/40 bg-surface-container-low p-3.5">
                                    <div class="mb-1 flex items-center gap-1.5 text-xs font-medium text-on-surface-variant">
                                    ...
                                    <p class="text-lg font-black text-ink">{{ number_format($stats['collectors']) }}</p>
                                </div>
                            </div>
                        </div>
    ...
    <section class="border-y border-outline-variant/40 bg-white py-8" id="dampak">
        <div class="mx-auto max-w-7xl px-5 sm:px-8 lg:px-12">
            <div class="grid grid-cols-1 items-center gap-8 lg:grid-cols-12">
                <div class="grid grid-cols-3 gap-4 border-b border-outline-variant/40 pb-6 sm:gap-6 lg:col-span-7 lg:border-b-0 lg:border-r lg:pb-0 lg:pr-8">
                    <div>
                        <p class="font-display text-2xl font-black tracking-tight text-ink sm:text-3xl">{{ number_format($stats['residents']) }}</p>
                        <p class="mt-1 text-xs font-semibold text-on-surface-variant sm:text-sm">Warga terdaftar</p>
                    </div>
                    <div>
                        ...
                        <p class="mt-1 text-xs font-semibold text-on-surface-variant sm:text-sm">Sampah terolah</p>
                    </div>
                    <div>
                        <p class="font-display text-2xl font-black tracking-tight text-ink sm:text-3xl">{{ number_format($stats['collectors']) }}</p>
                        <p class="mt-1 text-xs font-semibold text-on-surface-variant sm:text-sm">Kolektor aktif</p>
                    </div>
                </div>
                <div class="lg:col-span-5" id="mitra">
                    <span class="mb-3 block text-[11px] font-black uppercase tracking-widest text-outline">Mendukung agenda keberlanjutan global</span>
                    <div class="flex flex-wrap items-center gap-2 text-xs font-bold text-on-surface sm:gap-3">
                        <span class="inline-flex items-center gap-1.5 rounded-lg border border-outline-variant/40 bg-surface-container-low px-3 py-1.5"><span class="h-2 w-2 rounded-full bg-amber-500"></span> SDG 11 Kota</span>
                        <span class="inline-flex items-center gap-1.5 rounded-lg border border-outline-variant/40 bg-surface-container-low px-3 py-1.5"><span class="h-2 w-2 rounded-full bg-orange-500"></span> SDG 12 Konsumsi</span>
                        <span class="inline-flex items-center gap-1.5 rounded-lg border border-outline-variant/40 bg-surface-container-low px-3 py-1.5"><span class="h-2 w-2 rounded-full bg-emerald-600"></span> SDG 13 Aksi iklim</span>
                    </div>
                </div>
            </div>
        </div>
    ...
    <section class="bg-paper py-16 sm:py-24" id="cara-kerja">
        <div class="mx-auto max-w-7xl px-5 sm:px-8 lg:px-12">
            <div class="mb-12 max-w-2xl sm:mb-16">
                ...
                <h2 class="font-display text-3xl font-black tracking-tight text-ink sm:text-4xl">Bagaimana TemJi mengubah sampahmu jadi berkah</h2>
                <p class="mt-3 text-base text-on-surface-variant">Tiga langkah terstruktur dari rumah tangga hingga penukaran insentif nyata.</p>
            </div>
            <div class="grid grid-cols-1 gap-8 md:grid-cols-3">
                <div class="group rounded-3xl border border-outline-variant/40 bg-white p-8 transition-all duration-300 hover:border-brand-200 hover:shadow-lg">
                    ...
                    <h3 class="mb-3 text-xl font-bold text-ink">Pilah dari rumah</h3>
                    <p class="text-sm leading-relaxed text-on-surface-variant">Pisahkan sampah anorganik berdasarkan material seperti plastik, kertas, kaleng logam, dan kaca bersih.</p>
                </div>
                <div class="group rounded-3xl border border-outline-variant/40 bg-white p-8 transition-all duration-300 hover:border-brand-200 hover:shadow-lg">
                    ...
                    <h3 class="mb-3 text-xl font-bold text-ink">Jemput atau setor</h3>
                    <p class="text-sm leading-relaxed text-on-surface-variant">Kolektor terverifikasi TemJi mengambil ke rumah sesuai jadwal, atau kamu setor mandiri ke drop-point.</p>
                </div>
                <div class="group rounded-3xl border border-outline-variant/40 bg-white p-8 transition-all duration-300 hover:border-brand-200 hover:shadow-lg" id="reward">
                    ...
                    <h3 class="mb-3 text-xl font-bold text-ink">Dapatkan reward</h3>
                    <p class="text-sm leading-relaxed text-on-surface-variant">Dapatkan struk digital berpoin. Tukarkan dengan saldo e-wallet, pulsa, atau barang dari katalog.</p>
                </div>
            </div>
        </div>
    </section>

    <section class="border-t border-outline-variant/40 bg-white py-16 sm:py-20" id="kategori">
        <div class="mx-auto max-w-7xl px-5 sm:px-8 lg:px-12">
            <div class="mb-10 flex flex-col justify-between gap-4 md:mb-12 md:flex-row md:items-end">
                <div>
                    ...
                    <h2 class="font-display text-3xl font-black tracking-tight text-ink">Kategori sampah diterima</h2>
                </div>
                <p class="text-sm font-medium text-on-surface-variant">Nilai poin dihitung per kilogram dan disesuaikan dengan kondisi pemilahan</p>
            </div>
            <div class="grid grid-cols-2 gap-4 sm:grid-cols-3 sm:gap-5 lg:grid-cols-5">
                @forelse ($wasteCategories as $i => $category)
                    <div class="group rounded-2xl border border-outline-variant/40 bg-paper p-5 transition-all hover:border-brand-300 hover:bg-white hover:shadow-md sm:p-6">
                        ...
                        <h4 class="mb-1 text-base font-bold text-ink">{{ $category->name }}</h4>
                ...
                @empty
                    <p class="col-span-full text-sm text-on-surface-variant">Kategori sampah belum ditambahkan admin.</p>
                @endforelse
            </div>
        </div>
    </section>
    ...
    @if ($rewards->isNotEmpty())
        <section class="bg-paper py-16">
            <div class="mx-auto max-w-7xl px-5 sm:px-8 lg:px-12">
                <h2 class="font-display text-3xl font-black tracking-tight text-ink">Tukar poin jadi reward</h2>
                <p class="mt-2 max-w-lg text-sm text-on-surface-variant">Sebagian reward yang sedang tersedia untuk ditukar warga.</p>
